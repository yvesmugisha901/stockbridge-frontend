...
export default function AccountantDashboard() {
  ...
  useEffect(() => {
    async function load() {
      try {
        setLoading(true)
        // Fetch in parallel
        const [trfData, sumData] = await Promise.all([
          getTransfers(),
          getFinanceSummary(),
        ])
  ...
  const pendingCost     = transfers.filter(t =>
    ["HO_APPROVED", "IN_TRANSIT", "RECEIVED"].includes(t.status)
  ).length
  const totalCostRWF    = summary?.totalCost != null
  ...
  // Show last 4 transfers in the recent table
    ...
    <div style={{ maxWidth: 900 }}>
      ...
      {error && (
        <div style={{
          ...
          borderRadius: 7, padding: "10px 16px", marginBottom: 20,
        ...
        {STATS.map(({ label, value, sub, icon: Icon, color, bg }) => (
          <div key={label} style={{
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
              <span style={{ fontFamily: "'Inter', system-ui, sans-serif", fontSize: 12, color: "#6b7260" }}>
                {label}
              </span>
              <span style={{
            ...
            <p style={{ fontFamily: "'DM Serif Display', Georgia, serif", fontSize: 22, color: "#1a1f0e", margin: "0 0 4px" }}>
              {value}
            </p>
            <p style={{ fontFamily: "'Inter', system-ui, sans-serif", fontSize: 11, color: "#9ca3af", margin: 0 }}>
              {sub}
            </p>
          </div>
        ))}
      ...
      <div style={{ background: "#fff", border: "1px solid #e8ebe3", borderRadius: 10 }}>
        <div style={{
          ...
          <span style={{ fontFamily: "'Inter', system-ui, sans-serif", fontSize: 13, fontWeight: 500, color: "#1a1f0e" }}>
            Recent Transfers
          </span>
          <Link href="/accountant/transfers" style={{
            display: "flex", alignItems: "center", gap: 5,
            fontFamily: "'Inter', system-ui, sans-serif", fontSize: 12,
            color: "#3d7a2b", textDecoration: "none",
          }}>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={6} style={{ padding: "32px 16px", textAlign: "center", color: "#9ca3af", fontFamily: "'Inter', sans-serif", fontSize: 13 }}>
                  Loading…
                </td>
              </tr>
            ) : recent.length === 0 ? (
              ...
              // totalValue comes from TransferResponse
              const displayValue = r.totalValue != null ? `RWF ${Number(r.totalValue).toLocaleString()}` : "—"
              return (
