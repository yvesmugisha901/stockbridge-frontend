...
// ── StockAdjustmentRequest (exact DTO match) ───────────────────────────────
// itemId             Long     @NotNull
// branchId           Long     @NotNull
// adjustmentQuantity Integer  @NotNull  ← positive = ADD, negative = DEDUCT
...
export default function StockAdjustPage() {
  ...
  // Derived — selected item for unit label
  const selectedItem = items.find(i => String(i.id) === String(itemId)) ?? null

  // Load items + branches in parallel
  useEffect(() => {
    async function load() {
      try {
        setLoadingData(true)
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      await api.post("/stock/adjust", {
        ...
        adjustmentQuantity: adjustType === "ADD" ? qty : -qty,  // signed integer
        reason:             reason.trim(),
      })
  ...
  if (loadingData) {
    return (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 12, padding: "72px 0" }}>
        ...
        <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, color: "#6b7260", textTransform: "uppercase" }}>
          Loading items and branches...
        </span>
      ...
      <div style={{ background: "#fff", border: "1px solid #dde0d4", padding: "28px 28px 24px" }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            ...
            {/* StockAdjustmentRequest.itemId */}
            <Field label="Item" required>
              <select
                required
                value={itemId}
                onChange={e => setItemId(e.target.value)}
                style={{ ...inputBase, background: "#f7f8f4" }}
                onFocus={focus}
                onBlur={blur}
              >
            ...
            {/* StockAdjustmentRequest.branchId */}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            ...
            {/* Determines sign of adjustmentQuantity */}
                ...
                {["ADD", "DEDUCT"].map(type => (
                  <button
                    ...
                    style={{
                      ...
                      ...(adjustType === type
                        ? type === "ADD"
                          ? { background: "#f0f7ed", color: "#3d7a2b", borderColor: "#3d7a2b" }
            ...
            {/* StockAdjustmentRequest.adjustmentQuantity (absolute — sign applied on submit) */}
            <Field
              label={`Quantity${selectedItem?.unitOfMeasure ? ` (${selectedItem.unitOfMeasure})` : ""}`}
              required
