...
export default function InventoryPage() {
  ...
  // branches: BranchResponse[]  from GET /api/v1/branches  — needed for adjustment target
  const [branches, setBranches]       = useState([])
  const [loading, setLoading]         = useState(true)
  const [error, setError]             = useState(null)

  // Adjustment panel state
  const [selectedItem, setSelectedItem] = useState(null) // ItemResponse
  const [adjustBranchId, setAdjustBranchId] = useState("") // maps to StockAdjustmentRequest.branchId
  const [adjustQty, setAdjustQty]           = useState("") // raw input; sign applied below
  const [adjustType, setAdjustType]         = useState("ADD") // "ADD" | "DEDUCT"
  const [adjustReason, setAdjustReason]     = useState("") // StockAdjustmentRequest.reason
  const [submitting, setSubmitting]         = useState(false)

  // Load item catalogue — GET /api/v1/items
  // Returns: ApiResponse<Page<ItemResponse>>
  // ItemResponse fields: id, name, code, category, unitOfMeasure, unitPrice, active/status
  useEffect(() => {
    async function load() {
      try {
        setLoading(true)
  ...
  //   itemId: Long,
  //   branchId: Long,
  //   adjustmentQuantity: Integer,  ← positive for ADD, negative for DEDUCT
  //   reason: String
  // }
  async function handleApplyAdjustment(e) {
    ...
    try {
      setSubmitting(true)
      // adjustmentQuantity is signed: positive = ADD, negative = DEDUCT
      const adjustmentQuantity = adjustType === "ADD" ? qty : -qty

      await api.post("/stock/adjust", {
        itemId:             selectedItem.id,   // StockAdjustmentRequest.itemId
        branchId:           Number(adjustBranchId), // StockAdjustmentRequest.branchId
        adjustmentQuantity,                    // StockAdjustmentRequest.adjustmentQuantity
        reason:             adjustReason.trim(), // StockAdjustmentRequest.reason
      })
          ...
          <div style={{ background: "#fff", border: "1px solid #dde0d4", padding: "24px" }}>
            ...
            {items.length === 0 ? (
              ...
              items.map((entry, idx) => (
                <div
                  key={entry.id}
                  style={{ display: "grid", gridTemplateColumns: "120px 1.5fr 1fr 100px 110px 100px 90px", padding: "12px 0", borderBottom: idx < items.length - 1 ? "1px solid #f0f1ec" : "none", gap: 8, alignItems: "center" }}
                >
                  {/* ItemResponse.code */}
                  ...
                  {/* ItemResponse.unitPrice (BigDecimal) */}
                  ...
                  {/* ItemResponse.active (boolean) or .status (string) — handle both */}
                  <div>
          ...
          {selectedItem && (
            <div style={{ background: "#fff", border: "1px solid #3d7a2b", padding: "24px", display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 12, color: "#6b7260" }}>
                Adjusting:{" "}
                <strong style={{ color: "#1a1f0e", fontFamily: "'DM Mono', monospace" }}>
                  {selectedItem.code}
                </strong>{" "}
                — {selectedItem.name}
              </div>
              ...
              <form onSubmit={handleApplyAdjustment} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                ...
                {/* Branch selector — StockAdjustmentRequest.branchId */}
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  ...
                  >
                    ...
                    {branches.map((b) => (
                      // BranchResponse.id, BranchResponse.name
                      <option key={b.id} value={b.id}>{b.name}</option>
                    ))}
                  </select>
                </div>

                {/* Adjustment type — determines sign of StockAdjustmentRequest.adjustmentQuantity */}
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, textTransform: "uppercase", color: "#6b7260" }}>
                    Adjustment Type *
                  </span>
                ...
                {/* Quantity — StockAdjustmentRequest.adjustmentQuantity (absolute value; sign from type) */}
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, textTransform: "uppercase", color: "#6b7260" }}>
